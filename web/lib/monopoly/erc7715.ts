/**
 * BROWSER ERC-7715 "intuitive permission" grant for the BUY-IN / PAYMENT leg.
 *
 * This is the sip402 formula adapted for Nexus Monopoly. Instead of the player
 * blindly signing our opaque raw EIP-712 "Delegation" typed data (the budget
 * delegation in lib/delegations.ts), the player grants a spend authorization
 * through MetaMask's NATIVE permission popup — which renders the USDC token, the
 * per-period cap, the period duration and a human justification. MetaMask owns
 * the UX; we only declare intent.
 *
 * Flow (mirrors examples/uno/lib/erc7715.ts):
 ...
 * DelegationManager to pull the buy-in into the Pot (see lib/erc7715-settle.ts).
 *
 ...
 * covers the 0.05 USDC buy-in plus a buffer for replays/retries within the period.
 */
 ...
 */
export async function connectMetaMaskGrant(): Promise<{ owner: Address; grant: Erc7715Grant }> {
  const eth = injected();
  ...
  const grants = await walletClient.requestExecutionPermissions([
    {
      ...
      permission: {
        ...
        data: {
          tokenAddress: deployment.usdc,
          periodAmount: parseUnits(GRANT_CAP_USD, 6),
          periodDuration: PERIOD_DURATION_SECONDS,
          startTime: now,
          justification: `Spend up to ${GRANT_CAP_USD} USDC/day playing Nexus Monopoly`,
        },
