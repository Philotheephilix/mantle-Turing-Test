/**
 * SERVER-ONLY: generate + fund the MONOPOLY player keys (1 human + N bots), idempotently.
 *
 ...
 *
 * Writes examples/monopoly/players.local.json (gitignored). NEVER import from a
 * client component (it reads the relayer key).
 */
...
const PLAYERS_PATH = join(PLAYERS_BASE_DIR, "players.monopoly.local.json");
...
export async function ensurePlayers(opts: EnsurePlayersOptions = {}): Promise<PlayerKey[]> {
  ...
  for (const p of players) {
    ...
    if (haveEth < parseEther(ethEach)) {
      const ethHash = await nonceRetry(() => relayerWallet.sendTransaction({ account: relayer, chain: mantleSepoliaTestnet, to: p.address, value: parseEther(ethEach) }));
    ...
    //    (don't fail the run) — the player can still play with what it has. The relayer
    //    key is shared with the UNO example, so its USDC can be temporarily drained.
