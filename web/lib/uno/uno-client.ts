...
export class UnoClient {
  ...
  async pay(pot: Address, fee: string): Promise<{ ok: boolean; txHash?: Hex; error?: string }> {
    ...
    return this.post("/api/uno/charge", { player: this.account.address, signedBudget });
  }
  ...
  async grantSpend(grant: {
    ...
    return this.post("/api/uno/grant", {
  ...
  async payViaGrant(): Promise<{ ok: boolean; txHash?: Hex; error?: string }> {
    return this.post("/api/uno/charge", { player: this.account.address, grant: true });
  }
  ...
  async move(
    ...
    return this.post("/api/uno/move", { player: this.account.address, signedGameplay, kind, card, chosenColor });
  }
}
