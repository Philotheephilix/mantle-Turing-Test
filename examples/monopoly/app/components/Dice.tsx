...
function Face({ n, rolling }: { n: number; rolling: boolean }) {
  return (
    <div
      data-testid="die"
      className={`die w-12 h-12 sm:w-14 sm:h-14 rounded-xl grid p-1.5 ${rolling ? "animate-rolldice" : ""}`}
      ...
      {Array.from({ length: 9 }).map((_, i) => {
        ...
        return (
          <div key={i} className="flex items-center justify-center">
            {on && <div className="w-2 h-2 rounded-full bg-[#0b1220]" />}
          </div>
        );
      })}
    </div>
...
export function Dice({ die1, die2, rolling }: { die1: number; die2: number; rolling: boolean }) {
  return (
    <div className="flex items-center gap-3" data-testid="dice-tray">
      ...
      <div className="text-emerald-200/70 text-sm mono">
        = <span className="gold-text text-lg font-bold">{(die1 || 0) + (die2 || 0)}</span>
      </div>
    </div>
  );
}
