...
export function Card({
  ...
  const bg = wild ? "#101216" : COLOR_HEX[card.color] ?? "#3a3a3a";
  ...
  return (
    ...
    >
      <span className="absolute top-1.5 left-2 text-xs font-bold opacity-90">{g.corner}</span>
      <span
        className="absolute inset-0 flex items-center justify-center font-extrabold"
        style={{ textShadow: wild ? "0 0 12px rgba(255,255,255,0.3)" : "0 2px 0 rgba(0,0,0,0.18)" }}
      >
        {g.center}
      </span>
      <span className="absolute bottom-1.5 right-2 text-xs font-bold rotate-180 opacity-90">{g.corner}</span>
