...
/**
 * @title UnoPot
 * @notice A trustless USDC escrow per roomId for the Nexus UNO example.
 *
 *         The entry fee is a REAL x402 charge: each player's budget delegation is
 *         redeemed as `USDC.transferFrom(player -> this Pot, fee)`, so the USDC
 *         genuinely lands in this contract bounded by the player's on-chain spend
 *         caps and recipient allowlist. Because that transfer is driven through
 *         the delegation manager (manager is `msg.sender`), it does NOT pass
 *         through `deposit()` — so the settle ledger is recorded by the settle
 *         authority via `creditDeposit` (authority-gated, OFF the money path; it
 *         only mirrors a transfer that already happened on-chain).
 *
 ...
 *
 *         `deposit()` (the standard self-approve path) is retained for parity.
 */
contract UnoPot is ReentrancyGuard {
