...
export class MonopolyClient {
  ...
  state(): Promise<GameView> {
    return this.get("/api/monopoly/state");
  }
  ...
  async join(roomId: string, pot: Address): Promise<{ ok: boolean; txHash?: Hex; error?: string }> {
    ...
    return this.post("/api/monopoly/join", { player: this.address, signedGameplay, signedBudget });
  }
  ...
  async grantSpend(grant: {
    ...
    return this.post("/api/monopoly/grant", {
      player: this.address,
  ...
  async joinViaGrant(roomId: string): Promise<{ ok: boolean; txHash?: Hex; error?: string }> {
    ...
    return this.post("/api/monopoly/join", { player: this.address, signedGameplay, grant: true });
  }
  ...
  act(action: string, spaceId?: number): Promise<ActResult & GameView> {
    return this.post("/api/monopoly/act", { player: this.address, action, spaceId });
  }
}
