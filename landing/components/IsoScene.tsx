import { Cuboid, INK, project, pts, type Faces } from "./iso/primitives";
...
const C = {
  ...
  deck: { top: "oklch(0.7 0.2 30)", right: "oklch(0.6 0.19 29)", left: "oklch(0.5 0.17 28)" } as Faces,
  coin: { top: "oklch(0.85 0.15 85)", right: "oklch(0.74 0.15 80)", left: "oklch(0.64 0.14 76)" } as Faces,
  die: { top: "oklch(0.97 0.01 90)", right: "oklch(0.9 0.02 85)", left: "oklch(0.82 0.03 82)" } as Faces,
  wall: { top: "oklch(0.74 0.13 245)", right: "oklch(0.63 0.14 245)", left: "oklch(0.53 0.13 244)" } as Faces,
  wall2: { top: "oklch(0.66 0.15 300)", right: "oklch(0.56 0.15 300)", left: "oklch(0.47 0.13 299)" } as Faces,
  roof: { top: "oklch(0.68 0.18 30)", right: "oklch(0.58 0.17 29)", left: "oklch(0.49 0.15 28)" } as Faces,
  token: { top: "oklch(0.75 0.15 150)", right: "oklch(0.64 0.15 150)", left: "oklch(0.54 0.13 149)" } as Faces,
};
...
export function IsoScene({ className = "" }: { className?: string }) {
  ...
  return (
    ...
    >
        ...
        {(() => {
          ...
          return (
            <>
              ...
              <circle cx={cx[0]} cy={cx[1]} r={11} fill="none" stroke="oklch(0.6 0.19 29)" strokeWidth={3.5} />
            </>
          );
        })()}
      </g>
      ...
      <g className="float-b">
        {[0, 1, 2, 3, 4].map((i) => (
          <Cuboid key={i} x={1.5} y={-2.8} w={1.5} d={1.5} h={6.5} z={BZ + i * 6.5} faces={C.coin} />
        ))}
        {(() => {
          const [sx, sy] = project(2.25, -2.05, BZ + 5 * 6.5);
          return (
            <text x={sx} y={sy + 6} textAnchor="middle" fontFamily="var(--font-display)" fontWeight={800} fontSize={20} fill="oklch(0.5 0.14 76)">
              $
            </text>
          );
        })()}
